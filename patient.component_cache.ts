...
sourceWidth: 1200,
	...
	constructor(private httpClientService: HttpclientService, private authService: AuthService) {
		this.setPatientData = httpClientService.setPatientData;
	}
	...
	loadPatientData(){
		...
		if(localStorage.getItem('observations')){
			...
			this.handleSuccessfulResponse(observations);
			this.cptResponse(billing);
			...
			this.httpClientService.getDateRangeData(90,this.validity,this.mrn).subscribe(results => {
				...
				localStorage.setItem('observations',JSON.stringify(results[0]));
				localStorage.setItem('billing',JSON.stringify(results[1]));
	
				this.handleSuccessfulResponse(results[0]);
				this.cptResponse(results[1]);
				this.httpClientService.getPatients()
				.subscribe((resp:any) => {
					console.log('patinet resp..', resp);
					this.patients = resp;
					localStorage.setItem('patients',JSON.stringify(resp));
				});
			});
		}
  ...
  cptResponse(cptresponse){
	...
	let dateObject = this.httpClientService.getDates(this.dateRange); //30 by default, need to work if it will dynamic
	let compDate = this.getCompDate(dateObject, 30);
	
	console.log(compDate);

	if(cptresponse.billingAverage!='N/A' && cptresponse.devices.length && cptresponse.countOfObservationReviewed >=12 && cptresponse.lastBilledDate <= compDate){
		// Green tick
		this.greenTick = true;
