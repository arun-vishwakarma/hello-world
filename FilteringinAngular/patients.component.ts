...
export class PatientsComponent implements OnInit, OnDestroy {
  ...
  eventLogFilterOptions = {
    ...
    date: "",
    type: [],
    sbpValue: this.goal_sbp_value,
    dbpValue: this.goal_dbp_value,
    metClinicalProfileCriteria: "", //initially no filtering so let ""
    isValidDevice: "", //initially no filtering so let ""
    criteriaMet: [], //weekly average
    status: "", //HTN status
    minDate: {},
    maxDate: {},
    minDateDefault: {},
    mcpcCheckedYes: false,
    mcpcCheckedNo: false,
    deviceCheckedKnown: false,
    deviceCheckedUnKnown: false,
    statusActive: false, //HTN status
    statusInActive: false ////HTN status
  };
  eventLogModelStartDate: NgbDateStruct;
  eventLogModelEndDate: NgbDateStruct;
  ...
  loadPatientData(currentPatientObj = null) {
    ...
    this.httpClientService
      ...
      .subscribe(results => {
        ...
        if (this.eventLogFilterOptions.defaultStartDate == "")
          this.setEventLogData(results[0], results[3]);
    ...
    if (onlyDateRangeFilter) {
      this.httpClientService
        ...
        .subscribe(response => {
          ...
          this.setXAxisRange();
          //if (this.eventLogFilterOptions.defaultStartDate == "")
          //this.setEventLogData(response[0], response[2]); //on change date range set event log data
          this.handleSuccessfulResponse(response[0]); // changes after Rx Table introduced
        });
    } else {
      if (this.observationActualResponse) {
  ...
  /*
   ***********************  Event Log Functions Start *******************
   */
  setEventLogData(bpObserVation: any, htn: any, datePicker = false) {
    this.eventLogData = [];
    //console.log("bpObserVation", bpObserVation);
    //modify BP observation data as per event log data
    const bpObservations = this.assignEventDataObject(
      //remove null records from bp observation
      bpObserVation.observations.filter(data => {
        return data.sbpValue !== null;
      }),
      "BP Observation"
    );
    const weeklyAverages = this.assignEventDataObject(
      bpObserVation.averageObservations,
      "Weekly Average",
      bpObservations
    );
    const hyperTensionMedications = this.assignEventDataObject(
      htn,
      "Hypertension Medications"
    );
    this.eventLogData = this.eventLogData.concat(
      bpObservations,
      weeklyAverages,
      hyperTensionMedications
    );
    this.eventLogData.sort((a, b) => {
      return +new Date(b.date) - +new Date(a.date);
    });
    console.log("evevt log F", this.eventLogData, this.eventLogFilterOptions);

    if (!datePicker) {
      this.clearAllFilters();
      this.settingDatePickerValues();
    }
    this.filterEventLogData();
  }

  settingDatePickerValues() {
    this.defaultDateSetting();
    //Only Default (1st) time or when date range changed from view
    this.eventLogModelStartDate = this.settingBsDate(
      this.eventLogFilterOptions.startDate
    );
    this.eventLogModelEndDate = this.settingBsDate(
      this.eventLogFilterOptions.endDate
    );
    //fixed minDateDefault and every time change minDate when start date change from view
    this.eventLogFilterOptions.minDateDefault = this.eventLogFilterOptions.minDate = this.settingBsDate(
      this.eventLogFilterOptions.startDate
    );
    this.eventLogFilterOptions.maxDate = this.settingBsDate(
      this.eventLogFilterOptions.endDate
    );
  }

  defaultDateSetting() {
    //Default or 1st time set filter event option
    this.eventLogFilterOptions.startDate = this.datePipe.transform(
      this.minDate,
      "yyyy-MM-dd"
    );
    this.eventLogFilterOptions.endDate = this.datePipe.transform(
      this.maxDate,
      "yyyy-MM-dd"
    );

    //Default start end date fixed
    this.eventLogFilterOptions.defaultStartDate = this.datePipe.transform(
      this.minDate,
      "yyyy-MM-dd"
    );
    this.eventLogFilterOptions.defaultEndDate = this.datePipe.transform(
      this.maxDate,
      "yyyy-MM-dd"
    );
  }

  getWeeklyDeviceAndManufaturer(bpObservations, startDate, endDate) {
    const weeklyAvgDeviceAndManufacturer = bpObservations.filter(data => {
      return (
        data.date >= startDate &&
        data.date <= endDate &&
        data.type == "BP Observation" &&
        data.isValidDevice == "Y"
      );
    });
    const uniqueDeviceAndManufaturers = [
      ...new Set(
        weeklyAvgDeviceAndManufacturer.map(
          item => item.device.model + " " + item.device.manufacturer
        )
      )
    ];
    //console.log("week ddvdv", uniqueDeviceAndManufaturers);
    return uniqueDeviceAndManufaturers.length
      ? uniqueDeviceAndManufaturers.join(",")
      : "Unknown";
  }

  redBlackSbpAndDbp(sbpValue: number, dbpValue: number) {
    return sbpValue >= this.goal_sbp_value || dbpValue >= this.goal_dbp_value
      ? true
      : false;
  }

  settingBsDate(dateStr: string) {
  ...
  checkAndResetInputStartAndEndDate() {
    if (
      this.eventLogFilterOptions.startDate > this.eventLogFilterOptions.endDate
    ) {
      this.eventLogFilterOptions.endDate = this.eventLogFilterOptions.startDate;
    }
  }

  getEventLogBpObservationsAndHtn() {
    this.isLoaderOn = true;
    this.httpClientService
      .getObservationsAndHtn(
        this.eventLogFilterOptions.startDate,
        this.eventLogFilterOptions.endDate,
        this.mrn
      )
      .subscribe(result => {
        this.isLoaderOn = false;
        this.setEventLogData(result[0], result[1], true);
        //this.filterEventLogData();
      });
  }

  inputFilterStartDate(evt: any, v: string) {
    this.eventLogFilterOptions.startDate = v;
    this.eventLogFilterOptions.minDate = this.settingBsDate(v);
    this.checkAndResetInputStartAndEndDate();
    if (
      this.eventLogFilterOptions.startDate != "" &&
      this.eventLogFilterOptions.endDate != ""
    ) {
      this.clearAllFilters("date");
      this.getEventLogBpObservationsAndHtn();
      //this.filterEventLogData();
    }
  }

  inputFilterEndDate(evt: object, v: string) {
    this.eventLogFilterOptions.endDate = v;
    this.checkAndResetInputStartAndEndDate();
    if (
      this.eventLogFilterOptions.startDate != "" &&
      this.eventLogFilterOptions.endDate != ""
  ...
  inputFilterType(inp: KeyboardEvent, value: string) {
    if ((inp.target as HTMLInputElement).checked) {
      this.eventLogFilterOptions.type.push(value);
    } else {
      this.removeInputFilterType(value);
    }
    this.filterEventLogData();
  }
  removeInputFilterType(value: string, filterFlag = false) {
    this.eventLogFilterOptions.type.splice(
      this.eventLogFilterOptions.type.indexOf(value),
      1
    );
    //need to reset checkboxes as well
    if (value == "BP Observation") {
      this.clearMcpc();
      this.clearDevice();
    }
    if (value == "Weekly Average") {
      this.clearDevice();
      this.clearCriteriaMet();
    }
    if (value == "Hypertension Medications") {
      this.clearStatus();
    }

    if (filterFlag) this.filterEventLogData();
  }

  //weekly average type
  inputFilterCriteriaMet(inp: KeyboardEvent, value: string) {
    if ((inp.target as HTMLInputElement).checked) {
      this.eventLogFilterOptions.criteriaMet.push(value);
    } else {
      this.removeinputFilterCriteriaMet(value);
    }
    this.filterEventLogData();
  }
  removeinputFilterCriteriaMet(value: string) {
    this.eventLogFilterOptions.criteriaMet.splice(
      this.eventLogFilterOptions.criteriaMet.indexOf(value),
      1
    );
  }

  inputFilterMCPC(inp: KeyboardEvent) {
    if ((inp.target as HTMLInputElement).name == "mcpc-true") {
      this.eventLogFilterOptions.metClinicalProfileCriteria = "Y";
      this.eventLogFilterOptions.mcpcCheckedYes = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.mcpcCheckedYes &&
        this.eventLogFilterOptions.mcpcCheckedNo
      ) {
        this.eventLogFilterOptions.metClinicalProfileCriteria = "N";
      }
    }

    if ((inp.target as HTMLInputElement).name == "mcpc-false") {
      this.eventLogFilterOptions.metClinicalProfileCriteria = "N";
      this.eventLogFilterOptions.mcpcCheckedNo = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.mcpcCheckedNo &&
        this.eventLogFilterOptions.mcpcCheckedYes
      ) {
    ...
    if (
      ...
      this.eventLogFilterOptions.metClinicalProfileCriteria = "";
    }

    this.filterEventLogData();
  }

  inputFilterDevice(inp: KeyboardEvent) {
    if ((inp.target as HTMLInputElement).name == "device-known") {
      this.eventLogFilterOptions.isValidDevice = "Y";
      this.eventLogFilterOptions.deviceCheckedKnown = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.deviceCheckedKnown &&
        this.eventLogFilterOptions.deviceCheckedUnKnown
      ) {
        this.eventLogFilterOptions.isValidDevice = "N";
      }
    }

    if ((inp.target as HTMLInputElement).name == "device-unknown") {
      this.eventLogFilterOptions.isValidDevice = "N";
      this.eventLogFilterOptions.deviceCheckedUnKnown = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.deviceCheckedUnKnown &&
        this.eventLogFilterOptions.deviceCheckedKnown
      ) {
    ...
    if (
      ...
      this.eventLogFilterOptions.isValidDevice = "";
    }

    this.filterEventLogData();
  }

  //HTN (Hypertension Medications) status
  inputFilterStatus(inp: KeyboardEvent) {
    if ((inp.target as HTMLInputElement).name == "status-active") {
      this.eventLogFilterOptions.status = "active";
      this.eventLogFilterOptions.statusActive = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.statusActive &&
        this.eventLogFilterOptions.statusInActive
      ) {
        this.eventLogFilterOptions.status = "completed"; //completed, inactive
      }
    }

    if ((inp.target as HTMLInputElement).name == "status-inactive") {
      this.eventLogFilterOptions.status = "completed"; //completed, inactive
      this.eventLogFilterOptions.statusInActive = (inp.target as HTMLInputElement).checked;
      if (
        !this.eventLogFilterOptions.statusInActive &&
        this.eventLogFilterOptions.statusActive
      ) {
    ...
    if (
      ...
      this.eventLogFilterOptions.status = "";
    }

    this.filterEventLogData();
  }

  clearInputFilters(expr: string) {
    switch (expr) {
      case "date":
        this.defaultDateSetting();
        this.clearAllFilters("date");
        ...
        break;
      case "type":
        this.eventLogFilterOptions.type = [];
        this.clearMcpc();
        this.clearDevice();
        this.clearCriteriaMet();
        this.filterEventLogData();
      ...
      case "device":
        ...
        this.filterEventLogData();
        break;
      case "criteriaMet": //weekly average type
        this.clearCriteriaMet();
        this.filterEventLogData();
        break;
      case "status":
  ...
  clearMcpc() {
    this.eventLogFilterOptions.metClinicalProfileCriteria = "";
    this.eventLogFilterOptions.mcpcCheckedNo = this.eventLogFilterOptions.mcpcCheckedYes = false;
  }
  clearDevice() {
    this.eventLogFilterOptions.isValidDevice = "";
    this.eventLogFilterOptions.deviceCheckedKnown = this.eventLogFilterOptions.deviceCheckedUnKnown = false;
  }
  clearCriteriaMet() {
    this.eventLogFilterOptions.criteriaMet = [];
  }
  clearStatus() {
    this.eventLogFilterOptions.status = "";
    this.eventLogFilterOptions.statusActive = this.eventLogFilterOptions.statusInActive = false;
  }
  clearAllFilters(option: string = "") {
    if (option != "date") {
      this.eventLogFilterOptions.startDate = "";
      this.eventLogFilterOptions.endDate = "";
    }
  ...
  filterEventLogData() {
    //console.log("filter options..", this.eventLogFilterOptions);
    const dataByDate = this.filterEventLogByDate();
    const dataByType = this.filterEventLogByType(dataByDate);
    const dataByMcpc = this.filterEventLogByMcpc(dataByType);
    const dataByDevice = this.filterEventLogByDevice(dataByMcpc);
    const dataByCriteriaMet = this.filterEventLogByCriteriaMet(dataByDevice); //weekly average type
    const dataByStatus = this.filterEventLogByStatus(dataByCriteriaMet); //HTN
    this.eventLogDataList = dataByStatus;
    //console.log("list evt data ", this.eventLogDataList);
  }

  filterEventLogByDate() {
    if (
      this.eventLogFilterOptions.startDate != "" &&
      this.eventLogFilterOptions.endDate != ""
    ) {
      var eventLogData = this.eventLogData.filter(data => {
        return (
          data.date >= this.eventLogFilterOptions.startDate &&
          data.date <= this.eventLogFilterOptions.endDate
        );
      });
    } else {
      var eventLogData = this.eventLogData;
    }
    return eventLogData;
  }

  filterEventLogByType(filteredData: any) {
    if (this.eventLogFilterOptions.type.length) {
      var eventLogData = filteredData.filter(data => {
        return this.eventLogFilterOptions.type.includes(data.type);
      });
    } else {
      var eventLogData = filteredData;
    }
    return eventLogData;
  }

  filterEventLogByMcpc(filteredData: any) {
    if (this.eventLogFilterOptions.metClinicalProfileCriteria != "") {
      var eventLogData = filteredData.filter(data => {
        return (
          data.metClinicalProfileCriteria ==
            this.eventLogFilterOptions.metClinicalProfileCriteria ||
          !data.metClinicalProfileCriteria
        );
      });
  ...
  filterEventLogByDevice(filteredData: any) {
    if (this.eventLogFilterOptions.isValidDevice != "") {
      var eventLogData = filteredData.filter(data => {
        return (
          data.isValidDevice == this.eventLogFilterOptions.isValidDevice ||
          !data.isValidDevice
        );
      });
    } else {
  ...
  filterEventLogByCriteriaMet(filteredData: any) {
    if (this.eventLogFilterOptions.criteriaMet.length) {
      var eventLogData = filteredData.filter(data => {
        return (
          this.eventLogFilterOptions.criteriaMet.includes(data.criteriaMet) ||
          !data.criteriaMet
        );
      });
    } else {
  ...
  filterEventLogByStatus(filteredData: any) {
    if (this.eventLogFilterOptions.status.length) {
      var eventLogData = filteredData.filter(data => {
        return data.status == this.eventLogFilterOptions.status || !data.status;
      });
    } else {
      var eventLogData = filteredData;
    }
  ...
  assignEventDataObject(arr: Array<any>, type: string, bpOservations = null) {
    return arr.map(data => {
      var o = Object.assign({}, data);
      o.type = type;

      if (type == "BP Observation") {
        o.metClinicalProfileCriteria =
          data.isValid == "Y" &&
          (data.sbpValue !== null || data.sbpValue !== "")
            ? "Y"
            : "N";
      }

      if (type == "Weekly Average") {
        o.date = data.endTime;

        o.weeklyDeviceAndManufacturers =
          o.criteriaMet != "Inadequate data"
            ? this.getWeeklyDeviceAndManufaturer(
                bpOservations,
                data.startTime,
                data.endTime
              )
        ...
        o.isValidDevice = o.criteriaMet == "Inadequate data" ? "N" : "Y"; //override from client side, but should come from API ?
      }

      if (type == "Hypertension Medications") {
        o.date = data.effectiveDate;
      }
